 ...
 * 查询单条示例数据。
 */
export const getExampleItem = async (
  request: Request,
  response: Response,
  next: NextFunction,
): Promise<void> => {
...
export const createExampleItem = async (
  ...
  try {
    ...
    sendSuccess(response, data, "示例创建成功", 201);
  } catch (error) {
    next(error);
  }
};
...
export const updateExampleItem = async (
  ...
  try {
    ...
    sendSuccess(response, data, "示例更新成功");
  } catch (error) {
    next(error);
  }
};
 ...
 * 删除示例数据。
 */
export const removeExampleItem = async (
  request: Request,
  response: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    ...
    sendSuccess(response, data, "示例删除成功");
  } catch (error) {
    next(error);
  }
};
